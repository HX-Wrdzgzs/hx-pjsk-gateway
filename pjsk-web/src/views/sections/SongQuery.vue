  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex flex-wrap gap-1.5 mb-4">
      ...
      <button
        ...
        :key="quick.label"
        @click="searchQuery = quick.value; handleSearch()"
    ...
    <div v-if="isFetching" class="flex-1 flex flex-col items-center justify-center text-[#e28cb0]">
      ...
      <p class="text-xs font-mono tracking-widest animate-pulse">正在查询歌曲信息…</p>
    </div>
    ...
    <div v-else class="flex-1 flex flex-col items-center justify-center text-gray-400">
      ...
      <p class="text-sm font-bold text-gray-600">输入歌曲名称开始查询</p>
      <p class="text-[10px] mt-2 text-gray-400 text-center px-4">支持歌曲名、别名、歌曲 ID，例如：<span class="font-mono text-[#d4658b]">六兆年</span>、<span class="font-mono text-[#d4658b]">id123</span></p>
    </div>
  </div>
</template>
...
function buildCommand(query) {
  // 如果用户输入的是纯数字，当作歌曲 ID 处理
  const trimmed = query.trim()
  if (/^\d+$/.test(trimmed)) {
    return `pinfo ${props.serverPrefix}id${trimmed}`
  }
  return `pinfo ${props.serverPrefix}${trimmed}`
}
